        ...
        {낮은화면 ? (
          <>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', paddingBottom: '20px' }}>
              <h2 style={{ fontSize: `${28 * 배율}px`, fontWeight: 'bold', margin: '0 0 10px 0', color: fg }}>주문이 완료되었습니다.</h2>
              <p style={{ fontSize: `${18 * 배율}px`, color: 고대비 ? '#FFF' : '#555', margin: '0 0 30px 0' }}>영수증과 주문번호를 확인해주세요.</p>
              <div style={{ width: '280px', background: '#FFF', borderRadius: '12px', boxShadow: '0 6px 20px rgba(0,0,0,0.1)', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '30px 0', border: 고대비 ? `2px solid ${yellowColor}` : 'none' }}>
                <div style={{ fontSize: `${18 * 배율}px`, color: '#666', marginBottom: '10px' }}>주문번호</div>
                <div style={{ fontSize: `${60 * 배율}px`, fontWeight: 'bold', color: brownColor }}>{orderNumber}</div>
              </div>
              <button onClick={handleGoHome} style={{ marginTop: '30px', padding: '16px 50px', fontSize: `${22 * 배율}px`, fontWeight: 'bold', background: yellowColor, color: '#000', border: 'none', borderRadius: '30px', cursor: 'pointer' }}>
                처음 화면으로
              </button>
            </div>
          </>
          ...
          <>
            <div style={{ height: `${80 * 배율}px`, display: 'flex', alignItems: 'center', padding: '0 40px', background: 고대비 ? '#000000' : '#FFFFFF', flexShrink: 0 }}>
              <button onClick={handleGoHome} style={{ fontSize: `${44 * 배율}px`, background: 'transparent', border: 'none', cursor: 'pointer', padding: '0', color: '#000' }}>🏠</button>
            </div>
            <div style={{ padding: '30px 0', background: bg, flexShrink: 0 }}>
              <ProgressBar currentStep={4} 고대비={고대비} 배율={배율} />
            </div>
            <수어영역 안내텍스트="주문이 완료되었습니다." />
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
              <h2 style={{ fontSize: `${48 * 배율}px`, fontWeight: 'bold', margin: '0 0 20px 0', color: fg }}>주문이 완료되었습니다.</h2>
              <p style={{ fontSize: `${24 * 배율}px`, color: 고대비 ? '#FFF' : '#555', margin: '0 0 50px 0' }}>영수증과 주문번호를 확인해주세요.</p>
              <div style={{ width: '400px', background: '#FFF', borderRadius: '16px', boxShadow: '0 10px 30px rgba(0,0,0,0.1)', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '50px 0', border: 고대비 ? `2px solid ${yellowColor}` : 'none' }}>
                <div style={{ fontSize: `${24 * 배율}px`, color: '#666', marginBottom: '10px' }}>주문번호</div>
                <div style={{ fontSize: `${80 * 배율}px`, fontWeight: 'bold', color: brownColor }}>{orderNumber}</div>
              </div>
              <button onClick={handleGoHome} style={{ marginTop: '60px', padding: '20px 60px', fontSize: `${28 * 배율}px`, fontWeight: 'bold', background: yellowColor, color: '#000', border: 'none', borderRadius: '40px', cursor: 'pointer' }}>
                처음 화면으로
              </button>
            </div>
          </>
