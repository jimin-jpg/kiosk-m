...
export function PaymentScreen() {
  ...
  useEffect(() => {
    ...
    if (payStep === 'card' || payStep === 'easy') {
      timer = setTimeout(() => set화면('완료'), 3000);
    }
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', display: 'flex', flexDirection: 'column', background: 낮은화면 ? '#000000' : bg, color: fg, overflow: 'hidden' }}>
      ...
      <div style={{ flex: 낮은화면 ? '0 0 50%' : '1', display: 'flex', flexDirection: 'column', background: bg, overflow: 'hidden' }}>
        ...
        {/* 상단바 */}
        <div style={{ height: 낮은화면 ? '60px' : '80px', display: 'flex', alignItems: 'center', padding: '0 30px', background: '#FFFFFF', flexShrink: 0 }}>
          <button onClick={처음으로} style={{ fontSize: 낮은화면 ? '32px' : '44px', background: 'transparent', border: 'none', cursor: 'pointer', padding: '0', color: '#000' }}>🏠</button>
        </div>

        {/* 프로그레스바 - 일반 모드만 */}
        {!낮은화면 && (
          <div style={{ padding: '30px 0', background: bg, flexShrink: 0 }}>
            <ProgressBar currentStep={3} 고대비={고대비} />
          </div>
        )}
        ...
        {payStep === 'select' && (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', paddingBottom: '40px' }}>
            <h2 style={{ fontSize: 낮은화면 ? '24px' : '36px', fontWeight: 'bold', marginBottom: 낮은화면 ? '30px' : '60px', color: fg }}>결제수단을 선택해주세요.</h2>
            <div style={{ display: 'flex', gap: '30px', width: '100%', justifyContent: 'center', padding: '0 20px' }}>
              <button onClick={() => 결제수단선택('카드')} style={{ flex: 1, maxWidth: '280px', height: 낮은화면 ? '180px' : '280px', background: cardBg, borderRadius: '16px', border: cardBorder, boxShadow: cardShadow, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '20px', cursor: 'pointer' }}>
                <div style={{ fontSize: 낮은화면 ? '50px' : '80px' }}>💳</div>
                <span style={{ fontSize: 낮은화면 ? '20px' : '28px', fontWeight: 'bold', color: fg }}>카드결제, 삼성페이</span>
              </button>
              <button onClick={() => 결제수단선택('간편결제')} style={{ flex: 1, maxWidth: '280px', height: 낮은화면 ? '180px' : '280px', background: cardBg, borderRadius: '16px', border: cardBorder, boxShadow: cardShadow, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '20px', cursor: 'pointer' }}>
                <div style={{ fontSize: 낮은화면 ? '50px' : '80px' }}>📱</div>
                <span style={{ fontSize: 낮은화면 ? '20px' : '28px', fontWeight: 'bold', color: fg }}>간편결제</span>
              </button>
            </div>
          </div>
        )}
        ...
        {payStep === 'card' && (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', paddingBottom: '20px' }}>
            <h2 style={{ fontSize: 낮은화면 ? '26px' : '42px', fontWeight: 'bold', marginBottom: '40px', color: fg }}>카드를 투입구에 넣어주세요.</h2>
            ...
            <button onClick={() => setPayStep('select')} style={{ marginTop: '50px', padding: '16px 60px', background: '#777', color: '#FFF', fontSize: '24px', fontWeight: 'bold', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>이전</button>
          </div>
        )}
        ...
        {payStep === 'easy' && (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', paddingBottom: '20px' }}>
            <h2 style={{ fontSize: 낮은화면 ? '24px' : '36px', fontWeight: 'bold', marginBottom: '40px', textAlign: 'center', lineHeight: 1.5, color: fg }}>
            ...
            <button onClick={() => setPayStep('select')} style={{ marginTop: '40px', padding: '16px 60px', background: '#777', color: '#FFF', fontSize: '24px', fontWeight: 'bold', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>이전</button>
          </div>
        )}

        <div style={{ height: '120px', background: '#FFFFFF', flexShrink: 0 }}></div>
      </div>
    </div>
  );
}